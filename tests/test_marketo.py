...
class MarketoTestCase(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.mkto = sync.marketo.MarketoClient(sync.app.config['IDENTITY_ENDPOINT'], sync.app.config['CLIENT_ID'],
                                         sync.app.config['CLIENT_SECRET'], sync.app.config['API_ENDPOINT'])
